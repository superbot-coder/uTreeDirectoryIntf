...
Это интерфейс который строит Дерево файлов и каталогов из указанной директории.  По сути это тоже что было в предыдущем посте, но только это теперь интерфейс. 
добавлено:
- фильтрация по расширениям файлов
- построение списка с полными путями файлов

🛠 Управление индексами иконок
Добавил метод для управления иконками
    function SetIndexIcons(const IndexDir, indexFile: Integer): ITReeBuilder;
Если индексы иконок не задавались, от они по умолчанию не отображаются
